...
namespace Parking_App_WPF
{
    class User
    {
        ...
        {
            string query = String.Format("SELECT LicensePlate, created FROM guests WHERE Resident='{0}'", UID);
            ...
            foreach(DataRow row in dt.Rows)
            {
                ...
                DateTime created = row["created"] == DBNull.Value ? DateTime.Parse("0") : DateTime.Parse((String)row["created"]);
